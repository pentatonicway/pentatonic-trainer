...
function makeBox(
  root: BoxData["chordRoot"],
  quality: BoxData["chordQuality"]
): BoxData {
  return {
    id: `b-${root}`,
    chordRoot: root,
...
function renderLib(
  progs: Progression[] = [],
  overrides: Partial<React.ComponentProps<typeof ProgressionLibrary>> = {}
) {
  const cbs = {
    onLoad: vi.fn(),
    onDelete: vi.fn(),
...
describe("ProgressionLibrary", () => {
  ...
  it("renders progression thumbnail prominently", () => {
    ...
    expect(screen.getByTestId("prog-thumbnail-p1").textContent).toBe(
      "Am–Dm–E7"
    );
  });
  ...
  it("renders multiple progressions with correct thumbnails", () => {
    ...
    expect(screen.getByTestId("prog-thumbnail-p2").textContent).toBe(
      "A7–D7–E7"
    );
  });
  ...
  it("Confirming rename calls onRename with id and new name", () => {
    ...
    fireEvent.click(screen.getByTestId("rename-btn-p1"));
    fireEvent.change(screen.getByTestId("rename-input-p1"), {
