...
function rootOnlyVisible(): Record<ScaleDegree, boolean> {
  return Object.fromEntries(ALL_DEGREES.map((d) => [d, d === "1"])) as Record<
    ScaleDegree,
    boolean
  >;
}
...
export const useSessionStore = create<SessionState & SessionActions>(
  (set, get) => ({
    ...
    loadPreset(preset) {
      set((state) => {
        const recalced = recalcAllShapes(preset.boxes, state.overrides).map(
          (b) => ({ ...b, scaleDegreeVisibility: rootOnlyVisible() })
        );
        return {
          boxes: recalced,
          originalRoots: preset.boxes.map((b) => b.chordRoot),
          transposeOffset: 0,
          currentProgressionName: preset.name,
          currentSearchTerm: preset.searchTerm ?? "",
        };
      });
    },
    ...
    loadSavedProgression(id) {
      ...
      if (!prog) return;
      const recalced = recalcAllShapes(prog.boxes, state.overrides).map(
        (b) => ({ ...b, scaleDegreeVisibility: rootOnlyVisible() })
      );
      set({
        boxes: recalced,
