...
import type { BoxData, ChordQuality, NoteRoot, OverrideRule, ScaleDegree, Progression } from '../types'
import { resolveShapeIndex } from '../utils/shapeSelector'
import { getInterval, getIntervalSigned, transposeNote, transposeShape } from '../utils/transpose'
import { AUTO_SHAPE_OVERRIDES } from '../constants/AUTO_SHAPE_OVERRIDES'
import { getBaseShapes } from '../constants/shapes'
import { buildRuleKey, addOrReplaceOverride, removeOverride as removeOverrideFn } from '../utils/overrides'
import { useToastStore } from './toastStore'
import { loadSavedProgressions, saveProgressions, generateThumbnail } from '../utils/storage'
...
function allDegreesVisible(): Record<ScaleDegree, boolean> {
  return Object.fromEntries(ALL_DEGREES.map(d => [d, true])) as Record<ScaleDegree, boolean>
}

function rootOnlyVisible(): Record<ScaleDegree, boolean> {
  return Object.fromEntries(ALL_DEGREES.map(d => [d, d === '1'])) as Record<ScaleDegree, boolean>
}
...
function resolveAutoIndex(
  ...
  const box1TransposedShape = transposeShape(box1BaseShape, getIntervalSigned('A', box1.chordRoot))
  const box1DotFrets = box1TransposedShape.strings.flatMap(s => s.dots.map(d => d.fret))
  const box1BaseFret = Math.round((Math.min(...box1DotFrets) + Math.max(...box1DotFrets)) / 2)
...
function recalcUnlockedShapes(boxes: BoxData[], overrides: OverrideRule[]): BoxData[] {
 ...
 */
function recalcAllShapes(boxes: BoxData[], overrides: OverrideRule[]): BoxData[] {
  if (boxes.length === 0) return boxes
  const box1 = boxes[0]
  return boxes.map((box, i) => {
    if (i === 0) return { ...box, shapeIndex: resolveBox1ShapeIndex(box.chordQuality) }
    return { ...box, shapeIndex: resolveAutoIndex(box, box1, overrides) }
  })
}
 ...
 */
function recalcLockedShapes(boxes: BoxData[], overrides: OverrideRule[]): BoxData[] {
  if (boxes.length === 0) return boxes
  const box1 = boxes[0]
  return boxes.map((box, i) => {
    if (i === 0) return box
...
function makeInitialBox(): BoxData {
  return {
    ...
    transposeOffset: 0,
    locked: true,
    scaleDegreeVisibility: rootOnlyVisible(),
  }
}
...
export const useSessionStore = create<SessionState & SessionActions>((set, get) => ({
  ...
  setChordRoot(id, root) {
    set(state => {
      const updated = state.boxes.map(b => b.id === id ? { ...b, chordRoot: root } : b)
  ...
  setChordQuality(id, quality) {
    set(state => {
      const updated = state.boxes.map((b, i) => {
        if (b.id !== id) return b
        const shapeIndex = i === 0 ? resolveBox1ShapeIndex(quality) : b.shapeIndex
  ...
  selectChord(id, root, quality) {
    set(state => {
      ...
      const updated = state.boxes.map((b, i) => {
        ...
        const shapeIndex = i === 0
          ? resolveBox1ShapeIndex(quality)
          : resolveAutoIndex({ ...b, chordRoot: root, chordQuality: quality }, box1, state.overrides)
      ...
      const recalced = boxIndex === 0
        ? updated.map((box, i) => {
            if (i === 0) return box
            return { ...box, shapeIndex: resolveAutoIndex(box, newBox1, state.overrides) }
          })
        : updated
  ...
  resetShape(id) {
    set(state => {
      ...
      if (boxIndex === 0) {
        const updated = state.boxes.map(b =>
          b.id === id ? { ...b, shapeIndex: resolveBox1ShapeIndex(b.chordQuality) } : b
        )
        return { boxes: recalcLockedShapes(updated, state.overrides) }
      }
      const box1 = state.boxes[0]
      return {
        boxes: state.boxes.map(b => {
          if (b.id !== id) return b
          return { ...b, shapeIndex: resolveAutoIndex(b, box1, state.overrides) }
        }),
      }
    })
  },
  ...
  setGlobalDegree(degree, visible) {
    set(state => ({
      boxes: state.boxes.map(b => ({
        ...b,
        scaleDegreeVisibility: { ...b.scaleDegreeVisibility, [degree]: visible },
      })),
    }))
  },
  ...
  loadPreset(preset) {
    set(state => {
      const recalced = recalcAllShapes(preset.boxes, state.overrides)
        .map(b => ({ ...b, scaleDegreeVisibility: rootOnlyVisible() }))
      return {
        boxes: recalced,
  ...
  saveCurrentProgression(name) {
    set(state => {
      ...
      try { useToastStore.getState().showToast('Progression saved! ✓', 'success') } catch {}
      return { savedProgressions: updated, currentProgressionName: newProg.name }
    })
  },
  ...
  learnShape(boxId) {
    ...
    console.log(`[Admin Learn] Override saved: ${key} → shapeIndex ${box.shapeIndex}`)
    try { useToastStore.getState().showToast(`Override saved for ${key} ✓`, 'success') } catch {}
